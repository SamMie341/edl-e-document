...
import {
  DocumentTypeFilterParams,
  IDocumentTypeRepository,
} from '../../domain/repositories/document-type.repository.interface';
...
export class PrismaDocumentTypeRepository implements IDocumentTypeRepository {
  ...
  async create(data: any): Promise<DocumentType> {
    ...
    if (existing) {
      throw new ConflictException(
        `ລະຫັດ ຫຼື ຊື່ປະເພດເອກະສານນີ້ມີຢູ່ໃນລະບົບແລ້ວ`,
      );
    }
  ...
  async findById(id: string): Promise<DocumentType | null> {
    const model = await this.prisma.documentTypeModel.findUnique({
      where: { id },
    });
  ...
  async update(id: string, data: any): Promise<DocumentType> {
    const existing = await this.prisma.documentTypeModel.findUnique({
      where: { id },
    });
    if (!existing) {
      throw new NotFoundException('ບໍ່ພົບປະເພດເອກະສານນີ້ໃນລະບົບ');
    }
    const model = await this.prisma.documentTypeModel.update({
      where: { id },
      data,
    });
    return DocumentTypeMapper.toDomain(model);
