    ...
    async findAll(): Promise<Unit[]> {
        try {
            ...
            const response = await firstValueFrom(
                this.httpService.get(`${this.hrmUnitApiUrl}/unit`, { headers: { Authorization: `Bearer ${token}` } })
            );
            ...
            this.logger.log(`ບໍ່ສາມາດດຶງຂໍ້ມູນຈາກ HRMS ໄດ້: ${error.message}`, error.stack);
